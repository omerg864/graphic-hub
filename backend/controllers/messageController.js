...
const populate_receiver = {path: 'receiver', select: ['-password', '-__v', '-createdAt', '-updatedAt', '-verified', '-reset_token']};
...
const getChats = asyncHandler(async (req, res, next) => {
    ...
    }).populate(populate_sender).populate(populate_receiver).sort({createdAt: -1});
    const chats_sorted = [];
    const ids = []
    chats.forEach(chat => {
